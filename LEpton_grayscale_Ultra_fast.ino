...
void displayImage(void) {
  ...
  // Recherche min/max dans l'image (comme votre code original)
  ...
  // Utilisation directe des valeurs min/max (sans lissage AGC)
  double range = frame_max - frame_min;
  if (range < 100) range = 100; // Éviter division par zéro

  // Calcul des températures pour affichage
  max_temp = (frame_max / 100.0) - 273.15;
  min_temp = (frame_min / 100.0) - 273.15;

  // Rendu pixel par pixel avec conversion de couleur correcte
  for (int j = 0; j < image_y; j++) {
    for (int i = 0; i < image_x; i++) {
      // Normalisation basée sur les vraies valeurs min/max de la frame
      double norm = (image[i][j] - frame_min) / range;
      norm = constrain(norm, 0.0, 1.0);
      uint8_t grayValue = (uint8_t)(norm * 255.0);
      
      // Créer directement la couleur grise
      uint16_t gray565;
      if (grayValue == 0) {
        gray565 = 0x0000; // Noir
      } else if (grayValue == 255) {
        gray565 = 0xFFFF; // Blanc
      } else {
  ...
  // Barre d'infos : TEXTE BLANC SUR FOND NOIR
  uint16_t black = 0x0000; // Noir
  uint16_t white = 0xFFFF; // Blanc
  
  spr.fillRect(0, image_y - 8, image_x, 8, black);
  spr.setTextColor(white, black);
  spr.setTextSize(1);
  spr.setCursor(2, image_y - 7);

  char buffer[48];
  snprintf(buffer, sizeof(buffer), "%.1f-%.1fC  %.1fFPS", min_temp, max_temp, fps);
  spr.print(buffer);
