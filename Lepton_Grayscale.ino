...
void initIronBlackLUT() {
  for (int i = 0; i < 256; i++) {
    uint8_t v = 255 - i; // Inversion: 255 = froid (blanc), 0 = chaud (noir)
...
void displayImage(void) {
  ...
  // Dessine l'image dans la partie haute (y = 0 à 119)
  for (int i = 0; i < image_x; i++) {
    for (int j = 0; j < image_y; j++) {
      double norm = (image[i][j] - min) / (max - min);  // [0.0 - 1.0]
      uint8_t index = constrain(norm * 255.0, 0, 255);  // 0-255
      uint16_t color = ironBlackLUT[index];
      spr.drawPixel(i, j, color); // j reste < 120
    }
  }

  // Bandeau texte dans la zone du bas (y = 120 à 127)
  spr.fillRect(0, image_y, image_x, 8, TFT_WHITE); // fond blanc
  spr.setTextColor(TFT_BLACK, TFT_WHITE);          // texte noir sur fond blanc

  spr.setTextSize(1);
  spr.setCursor(2, image_y + 1); // y = 121
//
  char buffer[64];
  snprintf(buffer, sizeof(buffer), "Min: %.1fC Max: %.1fC", min_temp, max_temp);
  spr.print(buffer);

  // Affiche les FPS à droite
//  char fpsBuffer[16];
//  snprintf(fpsBuffer, sizeof(fpsBuffer), "FPS: %.1f", currentFPS);
//  int textWidth = spr.textWidth(fpsBuffer);
//  spr.setCursor(image_x - textWidth - 2, image_y + 1); // aligné à droite avec 2px de marge
//  spr.print(fpsBuffer);
  // Affiche tout sur l'écran (image + bandeau)
  spr.pushSprite(0, 0);
}
...
void setup() {
  ...
  spr.createSprite(image_x, image_y + 8); // Will have to update and show this twice
